...
def load_items(names_filter: List[str], limit: int, start: int, processed_ids: Set[int], category: Optional[str]) -> List[Dict]:
    ...
    if IGNORE_PATH.exists():
        try:
            ...
            pass
    todo = [t for t in todo if t['name'] not in ignore_names and t['name'].replace(' ', '_') not in ignore_names]
    ...
    def has_filtered(item):
        ...
        try:
            ...
            return isinstance(lines, list) and any(isinstance(ln, str) and ln.strip() for ln in lines)
        except Exception:
            return False
...
def build_prompt(batch: List[Dict], rules_text: str, output_path: Path) -> str:
    lines = []
    lines.append("You are updating Elden Ring item descriptions to match the 'Detailed Item Descriptions' mod style. Follow the formatting rules below exactly. Return JSON only, no prose.")
    lines.append("Caption = lore-only; Info = lore (if not already in caption) + mechanics per rules. Keep them distinct.")
    lines.append("Do not use placeholders; write final in-game text exactly as it should appear.")
    lines.append("If effect info is missing/uncertain or shows placeholders (e.g., '??'), resolve it: first read the provided HTML effect lines; if still unclear, look up the correct values online before writing.")
    lines.append("Do not include spoilers, quest/location guidance, acquisition steps, or patch notes—focus only on what the item does.")
    lines.append(f"Save the JSON array to this path: {output_path}")
    lines.append("Formatting rules:\n" + rules_text)
    lines.append("For each item, produce JSON with: id, name, category, caption (lore-only), info (final description with formatting). Keep lore intact, put mechanics per rules.")
    for idx, item in enumerate(batch, 1):
        wiki_effects = load_effect_lines_filtered(item['name'])
        lines.append(f"Item {idx}:")
        lines.append(f"id: {item['id']}, name: {item['name']}, category: {item['category']}, bundle: {item['bundle']}")
        ...
        lines.append("wiki_effect_lines:\n" + json.dumps(wiki_effects, ensure_ascii=False))
    lines.append("Output JSON array, each object: {\"id\": int, \"name\": str, \"category\": str, \"caption\": str, \"info\": str}.")
...
def run_batch(batch: List[Dict], rules_text: str, batch_idx: int, execute: bool, model: str, config: Path, save_raw: bool, save_full: bool, prefix: str, output_dir: Path):
    ...
    if parsed_obj is not None:
        ...
        print(f"[run] batch {batch_idx} exit {res.returncode}, parsed JSON -> {parsed_file}")
    else:
        if snippet:
...
def main():
    ...
    ap.add_argument('--limit', type=int, default=2, help='total items to process (for testing)')
    ap.add_argument('--names', nargs='*', help='specific item names to process')
    ap.add_argument('--execute', action='store_true', help='actually call codex-mcp-wrapper chat')
    ap.add_argument('--model', default='gpt-5.1-codex-mini')
    ap.add_argument('--config', default='../codex-mcp-wrapper/wrapper.toml')
    ap.add_argument('--save-raw', action='store_true', help='save extracted JSON snippet')
    ap.add_argument('--save-full', action='store_true', help='save full CLI output')
    ap.add_argument('--start', type=int, default=0, help='offset into todo list after filtering processed')
    ap.add_argument('--processed-glob', default='work/responses/ready/*_response*.json', help='glob of already processed response files to skip ids')
    ap.add_argument('--batch-prefix', default='', help='prefix for batch output files to avoid collisions')
    ap.add_argument('--category', help='restrict to category (armor, talisman, weapon, spell, skill, ash, consumable)')
    ap.add_argument('--output-dir', default='work/responses/pending', help='directory for responses (parsed/raw/full)')
    args = ap.parse_args()

    rules_text = FORMATTING_RULES.read_text(encoding='utf-8')
    processed_ids = load_processed_ids(args.processed_glob) if args.processed_glob else set()
    items = load_items(args.names or [], args.limit, args.start, processed_ids, args.category)
    ...
    for i in range(0, len(items), args.batch_size):
        ...
        if not batch:
            continue
        batch_idx += 1
        run_batch(batch, rules_text, batch_idx, args.execute, args.model, Path(args.config), args.save_raw, args.save_full, args.batch_prefix, Path(args.output_dir))
