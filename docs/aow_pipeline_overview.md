...
- Weapon collapse only proceeds when all non-numeric fields (excluding `Weapon`/`Weapon Source`) match across candidates; differing metadata prevents merging so per-weapon rows remain separate.
